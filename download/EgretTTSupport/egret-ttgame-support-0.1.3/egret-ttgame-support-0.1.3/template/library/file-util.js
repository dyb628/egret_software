...
const TT_ROOT = tt.env.USER_DATA_PATH + "/";

function walkFile(dirname, callback) {
    const files = ttFs.readdirSync(dirname)
    for (let f of files) {
        const file = dirname + "/" + f;
        const stat = ttFs.statSync(file);
        if (stat.isDirectory()) {
...
const fs = {
    ...
    remove: (dirname) => {
        if (!fs.existsSync(dirname))
            return;
        const globalDirname = TT_ROOT + dirname;
        walkFile(globalDirname, (file) => {
            ttFs.unlinkSync(file);
            let p = file.replace(TT_ROOT, "");
            p = path.normailze(p);
            if (fs_cache[p]) {
                fs_cache[p] = 0;
            }
    ...
    writeSync: (p, content) => {
        p = path.normailze(p);
        fs_cache[p] = 1;
        ttFs.writeFileSync(TT_ROOT + p, content);
    },

    readSync: (p, format) => {
        format = format || 'utf-8';
        return wxFs.readFileSync(TT_ROOT + p, format);
    },
        ...
        if (!fs.existsSync(p)) {
            ...
            for (let i = 0; i < dirs.length; i++) {
                ...
                if (!fs.existsSync(current)) {
                    ...
                    fs_cache[p] = 1;
                    wxFs.mkdirSync(TT_ROOT + current)
    ...
    unzip: (zipFilePath, targetPath) => {
        zipFilePath = TT_ROOT + zipFilePath;
        targetPath = TT_ROOT + targetPath;
