...
class ImageProcessor {
    ...
    onLoadStart(host, resource) {
        ...
        if (fs.existsSync(fullname)) {
            return loadImage(path.getTTUserPath(fullname), scale9Grid);
        } else {
...
function download(url, target) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const file_target = path.getTTUserPath(target);
        tt.downloadFile({
            url: url,
            ...
            success: (v) => {
                if (v.statusCode >= 400) {
                    try {
                        ttFs.accessSync(file_target);
                        ttFs.unlinkSync(file_target);
                    } catch (e) {
