...
_project(script){const prefix=script.isContentScript()?'js:extensions:':'js::';const projectId=prefix+this._debuggerModel.target().id()+':'+script.frameId;let project=this._projects.get(projectId);if(!project){const projectType=script.isContentScript()?Workspace.Workspace.projectTypes.ContentScripts:Workspace.Workspace.projectTypes.Network;project=new ContentProviderBasedProject(this._workspace,projectId,projectType,'',false);NetworkProject.setTargetForProject(project,this._debuggerModel.target());this._projects.set(projectId,project);}
return project;}
...
const script=scriptFile._script;if(script.isWasmDisassembly()){return[script.wasmByteLocation(lineNumber)];}
...
if(script.isContentScript()&&!script.hasSourceURL){const parsedURL=new Common.ParsedURL.ParsedURL(script.sourceURL);if(!parsedURL.isValid){return false;}}
return true;}
...
const debuggerModel=this._resourceScriptMapping._debuggerModel;const breakpoints=self.Bindings.breakpointManager.breakpointLocationsForUISourceCode(this._uiSourceCode).map(breakpointLocation=>breakpointLocation.breakpoint);const source=this._uiSourceCode.workingCopy();debuggerModel.setScriptSource(this._script.scriptId,source,scriptSourceWasSet.bind(this));async function scriptSourceWasSet(error,exceptionDetails){if(!error&&!exceptionDetails){this._scriptSource=source;}
this._update();if(!error&&!exceptionDetails){await Promise.all(breakpoints.map(breakpoint=>breakpoint.refreshInDebugger()));return;}
if(!exceptionDetails){self.Common.console.addMessage(Common.UIString.UIString('LiveEdit failed: %s',error),Common.Console.MessageLevel.Warning);return;}
const messageText=Common.UIString.UIString('LiveEdit compile failed: %s',exceptionDetails.text);this._uiSourceCode.addLineMessage(Workspace.UISourceCode.Message.Level.Error,messageText,exceptionDetails.lineNumber,exceptionDetails.columnNumber);}}
...
ResourceScriptFile.Events={DidMergeToVM:Symbol('DidMergeToVM'),DidDivergeFromVM:Symbol('DidDivergeFromVM'),};