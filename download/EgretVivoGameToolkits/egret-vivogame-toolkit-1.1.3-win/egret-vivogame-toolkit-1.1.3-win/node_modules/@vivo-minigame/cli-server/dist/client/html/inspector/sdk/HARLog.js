import*as Common from'../common/common.js';import{Cookie}from'./Cookie.js';import{PageLoad}from'./NetworkLog.js';import{NetworkRequest}from'./NetworkRequest.js';export class HARLog{static pseudoWallTime(request,monotonicTime){return new Date(request.pseudoWallTime(monotonicTime)*1000);}
...
if(harEntry._request.resourceType()===Common.ResourceType.resourceTypes.WebSocket){const messages=[];for(const message of harEntry._request.frames()){messages.push({type:message.type,time:message.time,opcode:message.opCode,data:message.text});}
