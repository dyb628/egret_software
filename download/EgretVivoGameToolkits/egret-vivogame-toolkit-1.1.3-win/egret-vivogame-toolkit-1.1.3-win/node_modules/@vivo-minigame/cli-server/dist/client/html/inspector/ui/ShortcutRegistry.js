import*as Host from'../host/host.js';import{Action}from'./Action.js';import{ActionRegistry}from'./ActionRegistry.js';import{Context}from'./Context.js';import{Dialog}from'./Dialog.js';import{Descriptor,KeyboardShortcut,Modifiers}from'./KeyboardShortcut.js';import{isEditing}from'./UIUtils.js';export class ShortcutRegistry{constructor(actionRegistry,document){this._actionRegistry=actionRegistry;this._defaultKeyToActions=new Platform.Multimap();this._defaultActionToShortcut=new Platform.Multimap();this._registerBindings(document);}
_applicableActions(key){return this._actionRegistry.applicableActions([...this._defaultActionsForKey(key)],self.UI.context);}
_defaultActionsForKey(key){return this._defaultKeyToActions.get(String(key));}
globalShortcutKeys(){const keys=[];for(const key of this._defaultKeyToActions.keysArray()){const actions=[...this._defaultKeyToActions.get(key)];const applicableActions=this._actionRegistry.applicableActions(actions,new Context());if(applicableActions.length){keys.push(Number(key));}}
return keys;}
shortcutDescriptorsForAction(actionId){return[...this._defaultActionToShortcut.get(actionId)];}
...
eventMatchesAction(event,actionId){console.assert(this._defaultActionToShortcut.has(actionId),'Unknown action '+actionId);const key=KeyboardShortcut.makeKeyFromEvent(event);return[...this._defaultActionToShortcut.get(actionId)].some(descriptor=>descriptor.key===key);}
...
for(const action of actions){try{const result=await action.execute();if(result){Host.userMetrics.keyboardShortcutFired(action.id());return;}}catch(e){console.error(e);throw e;}}
...
const modifiers=Modifiers;if(Host.Platform.isMac()){if(KeyboardShortcut.makeKey('z',modifiers.Meta)===key){return true;}
...
if(!Host.Platform.isWin()&&KeyboardShortcut.makeKey('z',modifiers.Ctrl|modifiers.Shift)===key){return true;}}
if((keyModifiers&(modifiers.Ctrl|modifiers.Alt))===(modifiers.Ctrl|modifiers.Alt)){return Host.Platform.isWin();}
...
registerShortcut(actionId,shortcut){const descriptor=KeyboardShortcut.makeDescriptorFromBindingShortcut(shortcut);if(!descriptor){return;}
...
const platforms=platformsString.split(',');let isMatch=false;const currentPlatform=Host.Platform.platform();for(let i=0;!isMatch&&i<platforms.length;++i){isMatch=platforms[i]===currentPlatform;}
return isMatch;}}}
