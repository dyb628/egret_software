...
async _updateModifiedState(uiSourceCode){this._loadingUISourceCodes.delete(uiSourceCode);if(uiSourceCode.project().type()!==Workspace.Workspace.projectTypes.Network){this._markAsUnmodified(uiSourceCode);return;}
...
export class UISourceCodeDiff extends Common.ObjectWrapper.ObjectWrapper{constructor(uiSourceCode){super();this._uiSourceCode=uiSourceCode;uiSourceCode.addEventListener(Workspace.UISourceCode.Events.WorkingCopyChanged,this._uiSourceCodeChanged,this);uiSourceCode.addEventListener(Workspace.UISourceCode.Events.WorkingCopyCommitted,this._uiSourceCodeChanged,this);this._requestDiffPromise=null;this._pendingChanges=null;this._dispose=false;}
...
export const Events={DiffChanged:Symbol('DiffChanged'),ModifiedStatusChanged:Symbol('ModifiedStatusChanged')};let _instance=null;export function workspaceDiff(){if(!_instance){_instance=new WorkspaceDiffImpl(self.Workspace.workspace);}
return _instance;}
