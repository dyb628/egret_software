import*as Common from'../common/common.js';import{HeapProfilerModel}from'./HeapProfilerModel.js';import{RuntimeModel}from'./RuntimeModel.js';import{SDKModelObserver}from'./SDKModel.js';export class IsolateManager extends Common.ObjectWrapper.ObjectWrapper{constructor(){super();console.assert(!self.SDK.isolateManager,'Use self.SDK.isolateManager singleton.');this._isolates=new Map();this._isolateIdByModel=new Map();this._observers=new Set();self.SDK.targetManager.observeModels(RuntimeModel,this);this._pollId=0;}
...
this._usedHeapSize=usage.usedSize;this._memoryTrend.add(this._usedHeapSize);self.SDK.isolateManager.dispatchEventToListeners(Events.MemoryChanged,this);}
