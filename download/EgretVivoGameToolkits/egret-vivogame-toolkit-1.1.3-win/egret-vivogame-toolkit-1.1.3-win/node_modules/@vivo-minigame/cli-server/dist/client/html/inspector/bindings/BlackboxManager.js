import*as Common from'../common/common.js';import*as SDK from'../sdk/sdk.js';import*as Workspace from'../workspace/workspace.js';import{DebuggerWorkspaceBinding}from'./DebuggerWorkspaceBinding.js';export class BlackboxManager{constructor(debuggerWorkspaceBinding){this._debuggerWorkspaceBinding=debuggerWorkspaceBinding;self.SDK.targetManager.addModelListener(SDK.DebuggerModel.DebuggerModel,SDK.DebuggerModel.Events.GlobalObjectCleared,this._clearCacheIfNeeded.bind(this),this);self.Common.settings.moduleSetting('skipStackFramesPattern').addChangeListener(this._patternChanged.bind(this));self.Common.settings.moduleSetting('skipContentScripts').addChangeListener(this._patternChanged.bind(this));this._listeners=new Set();this._isBlackboxedURLCache=new Map();self.SDK.targetManager.observeModels(SDK.DebuggerModel.DebuggerModel,this);}
...
_setBlackboxPatterns(debuggerModel){const regexPatterns=self.Common.settings.moduleSetting('skipStackFramesPattern').getAsArray();const patterns=([]);for(const item of regexPatterns){if(!item.disabled&&item.pattern){patterns.push(item.pattern);}}
return debuggerModel.setBlackboxPatterns(patterns);}
isBlackboxedUISourceCode(uiSourceCode){const projectType=uiSourceCode.project().type();const isContentScript=projectType===Workspace.Workspace.projectTypes.ContentScripts;if(isContentScript&&self.Common.settings.moduleSetting('skipContentScripts').get()){return true;}
...
if(isContentScript&&self.Common.settings.moduleSetting('skipContentScripts').get()){return true;}
const regex=self.Common.settings.moduleSetting('skipStackFramesPattern').asRegExp();const isBlackboxed=(regex&&regex.test(url))||false;this._isBlackboxedURLCache.set(url,isBlackboxed);return isBlackboxed;}
...
async _updateScriptRanges(script,sourceMap){let hasBlackboxedMappings=false;if(!self.Bindings.blackboxManager.isBlackboxedURL(script.sourceURL,script.isContentScript())){hasBlackboxedMappings=sourceMap?sourceMap.sourceURLs().some(url=>this.isBlackboxedURL(url)):false;}
...
return true;}}
_uiSourceCodeURL(uiSourceCode){return uiSourceCode.project().type()===Workspace.Workspace.projectTypes.Debugger?null:uiSourceCode.url();}
...
blackboxContentScripts(){self.Common.settings.moduleSetting('skipContentScripts').set(true);}
unblackboxContentScripts(){self.Common.settings.moduleSetting('skipContentScripts').set(false);}
_blackboxURL(url){const regexPatterns=self.Common.settings.moduleSetting('skipStackFramesPattern').getAsArray();const regexValue=this._urlToRegExpString(url);if(!regexValue){return;}
...
_unblackboxURL(url){let regexPatterns=self.Common.settings.moduleSetting('skipStackFramesPattern').getAsArray();const regexValue=self.Bindings.blackboxManager._urlToRegExpString(url);if(!regexValue){return;}
...
self.Common.settings.moduleSetting('skipStackFramesPattern').setAsArray(regexPatterns);}
async _patternChanged(){this._isBlackboxedURLCache.clear();const promises=[];for(const debuggerModel of self.SDK.targetManager.models(SDK.DebuggerModel.DebuggerModel)){promises.push(this._setBlackboxPatterns(debuggerModel));const sourceMapManager=debuggerModel.sourceMapManager();for(const script of debuggerModel.scripts()){promises.push(this._updateScriptRanges(script,sourceMapManager.sourceMapForClient(script)));}}
...
_urlToRegExpString(url){const parsedURL=new Common.ParsedURL.ParsedURL(url);if(parsedURL.isAboutBlank()||parsedURL.isDataURL()){return'';}
