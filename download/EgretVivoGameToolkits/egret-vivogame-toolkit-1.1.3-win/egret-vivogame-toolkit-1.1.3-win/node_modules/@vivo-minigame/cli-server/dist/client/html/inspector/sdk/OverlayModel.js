import*as Common from'../common/common.js';import{DebuggerModel,Events as DebuggerModelEvents}from'./DebuggerModel.js';import{DeferredDOMNode,DOMModel,DOMNode}from'./DOMModel.js';import{RemoteObject}from'./RemoteObject.js';import{Capability,SDKModel,Target}from'./SDKModel.js';export class OverlayModel extends SDKModel{constructor(target){super(target);this._domModel=(target.model(DOMModel));target.registerOverlayDispatcher(this);this._overlayAgent=target.overlayAgent();this._debuggerModel=target.model(DebuggerModel);if(this._debuggerModel){self.Common.settings.moduleSetting('disablePausedStateOverlay').addChangeListener(this._updatePausedInDebuggerMessage,this);this._debuggerModel.addEventListener(DebuggerModelEvents.DebuggerPaused,event=>{this._updatePausedInDebuggerMessage();},this);this._debuggerModel.addEventListener(DebuggerModelEvents.DebuggerResumed,event=>{this._updatePausedInDebuggerMessage();},this);this._debuggerModel.addEventListener(DebuggerModelEvents.GlobalObjectCleared,event=>{this._updatePausedInDebuggerMessage();},this);}
this._inspectModeEnabled=false;this._hideHighlightTimeout=null;this._defaultHighlighter=new DefaultHighlighter(this);this._highlighter=this._defaultHighlighter;this._showPaintRectsSetting=self.Common.settings.moduleSetting('showPaintRects');this._showLayoutShiftRegionsSetting=self.Common.settings.moduleSetting('showLayoutShiftRegions');this._showAdHighlightsSetting=self.Common.settings.moduleSetting('showAdHighlights');this._showDebugBordersSetting=self.Common.settings.moduleSetting('showDebugBorders');this._showFPSCounterSetting=self.Common.settings.moduleSetting('showFPSCounter');this._showScrollBottleneckRectsSetting=self.Common.settings.moduleSetting('showScrollBottleneckRects');this._showHitTestBordersSetting=self.Common.settings.moduleSetting('showHitTestBorders');this._registeredListeners=[];this._showViewportSizeOnResize=true;if(!target.suspended()){this._overlayAgent.enable();this._wireAgentToSettings();}}
...
static hideDOMNodeHighlight(){for(const overlayModel of self.SDK.targetManager.models(OverlayModel)){overlayModel._delayedHideHighlight(0);}}
static async muteHighlight(){return Promise.all(self.SDK.targetManager.models(OverlayModel).map(model=>model.suspendModel()));}
static async unmuteHighlight(){return Promise.all(self.SDK.targetManager.models(OverlayModel).map(model=>model.resumeModel()));}
...
suspendModel(){Common.EventTarget.EventTarget.removeEventListeners(this._registeredListeners);return this._overlayAgent.disable();}
...
const message=this._debuggerModel.isPaused()&&!self.Common.settings.moduleSetting('disablePausedStateOverlay').get()?Common.UIString.UIString('Paused in debugger'):undefined;this._overlayAgent.setPausedInDebuggerMessage(message);}
...
_buildHighlightConfig(mode='all',showStyles=false){const showRulers=self.Common.settings.moduleSetting('showMetricsRulers').get();const highlightConfig={showInfo:mode==='all',showRulers:showRulers,showStyles,showExtensionLines:showRulers};if(mode==='all'||mode==='content'){highlightConfig.contentColor=Common.Color.PageHighlight.Content.toProtocolRGBA();}
...
inspectNodeRequested(backendNodeId){const deferredNode=new DeferredDOMNode(this.target(),backendNodeId);if(OverlayModel._inspectNodeHandler){deferredNode.resolvePromise().then(node=>{if(node){OverlayModel._inspectNodeHandler(node);}});}else{Common.Revealer.reveal(deferredNode);}
...
SDKModel.register(OverlayModel,Capability.DOM,true);export let HighlightData;