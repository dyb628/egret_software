import*as ProtocolModule from'../protocol/protocol.js';import{DebuggerModel,FunctionDetails}from'./DebuggerModel.js';import{RuntimeModel}from'./RuntimeModel.js';export class RemoteObject{static fromLocalObject(value){return new LocalJSONObject(value);}
...
const response=await this._runtimeAgent.invoke_getProperties({objectId:this._objectId,ownProperties,accessorPropertiesOnly,generatePreview});if(response[ProtocolModule.InspectorBackend.ProtocolError]){return({properties:null,internalProperties:null});}
...
const response=await this._runtimeAgent.invoke_evaluate({expression:value,silent:true});if(response[ProtocolModule.InspectorBackend.ProtocolError]||response.exceptionDetails){return response[ProtocolModule.InspectorBackend.ProtocolError]||(response.result.type!=='string'?response.result.description:(response.result.value));}
...
async doSetObjectPropertyValue(result,name){const setPropertyValueFunction='function(a, b) { this[a] = b; }';const argv=[name,RemoteObject.toCallArgument(result)];const response=await this._runtimeAgent.invoke_callFunctionOn({objectId:this._objectId,functionDeclaration:setPropertyValueFunction,arguments:argv,silent:true});const error=response[ProtocolModule.InspectorBackend.ProtocolError];return error||response.exceptionDetails?error||response.result.description:undefined;}
async deleteProperty(name){if(!this._objectId){return'Can’t delete a property of non-object.';}
const deletePropertyFunction='function(a) { delete this[a]; return !(a in this); }';const response=await this._runtimeAgent.invoke_callFunctionOn({objectId:this._objectId,functionDeclaration:deletePropertyFunction,arguments:[name],silent:true});if(response[ProtocolModule.InspectorBackend.ProtocolError]||response.exceptionDetails){return response[ProtocolModule.InspectorBackend.ProtocolError]||response.result.description;}
if(!response.result.value){return'Failed to delete property.';}}
async callFunction(functionDeclaration,args){const response=await this._runtimeAgent.invoke_callFunctionOn({objectId:this._objectId,functionDeclaration:functionDeclaration.toString(),arguments:args,silent:true});if(response[ProtocolModule.InspectorBackend.ProtocolError]){return{object:null,wasThrown:false};}
return{object:this._runtimeModel.createRemoteObject(response.result),wasThrown:!!response.exceptionDetails};}
async callFunctionJSON(functionDeclaration,args){const response=await this._runtimeAgent.invoke_callFunctionOn({objectId:this._objectId,functionDeclaration:functionDeclaration.toString(),arguments:args,silent:true,returnByValue:true});return response[ProtocolModule.InspectorBackend.ProtocolError]||response.exceptionDetails?null:response.result.value;}
...
object(){return this._object;}}
const _descriptionLengthParenRegex=/\(([0-9]+)\)/;const _descriptionLengthSquareRegex=/\[([0-9]+)\]/;const UnserializableNumber={Negative0:('-0'),NaN:('NaN'),Infinity:('Infinity'),NegativeInfinity:('-Infinity')};export let CallFunctionResult;export let GetPropertiesResult;