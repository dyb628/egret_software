...
this._fileSelectorElement=UI.UIUtils.createFileSelectorElement(this._loadFromFile.bind(this));this.element.appendChild(this._fileSelectorElement);}
wasShown(){}
willHide(){}
...
async _saveToFile(){console.assert(this._state===State.Idle&&this._tracingModel);const fileName=`InputProfile-${new Date().toISO8601Compact()}.json`;const stream=new Bindings.FileUtils.FileOutputStream();const accepted=await stream.open(fileName);if(!accepted){return;}
...
_loadFromFile(file){console.assert(this._isAvailableState());this._setState(State.Loading);this._loader=Timeline.TimelineLoader.TimelineLoader.loadFromFile(file,this);this._createFileSelector();}
async _startRecording(){this._setState(State.StartPending);this._tracingClient=new InputTimeline.TracingClient((self.SDK.targetManager.mainTarget()),this);const response=await this._tracingClient.startRecording();if(response[ProtocolModule.InspectorBackend.ProtocolError]){this._recordingFailed(response[ProtocolModule.InspectorBackend.ProtocolError]);}else{this._setState(State.Recording);}}
...
this._inputModel=new InputModel((self.SDK.targetManager.mainTarget()));this._tracingModel=tracingModel;this._inputModel.setEvents(tracingModel);this._setState(State.Idle);}
_recordingFailed(error){this._tracingClient=null;this._setState(State.Idle);}
replayStopped(){this._setState(State.Idle);}}
export const State={Idle:Symbol('Idle'),StartPending:Symbol('StartPending'),Recording:Symbol('Recording'),StopPending:Symbol('StopPending'),Replaying:Symbol('Replaying'),ReplayPaused:Symbol('ReplayPaused'),Loading:Symbol('Loading')};export class ActionDelegate{handleAction(context,actionId){const inputViewId='Inputs';self.UI.viewManager.showView(inputViewId).then(()=>self.UI.viewManager.view(inputViewId).widget()).then(widget=>this._innerHandleAction((widget),actionId));return true;}
...
export class TracingClient{constructor(target,client){this._target=target;this._tracingManager=target.model(SDK.TracingManager.TracingManager);this._client=client;const backingStorage=new Bindings.TempFile.TempFileBackingStorage();this._tracingModel=new SDK.TracingModel.TracingModel(backingStorage);this._tracingCompleteCallback=null;}
...
await this._waitForTracingToStop(true);await self.SDK.targetManager.resumeAllTargets();this._tracingModel.tracingComplete();this._client.loadingComplete(this._tracingModel);}
