...
scheduleRefresh(){UI.UIUtils.invokeOnceAfterBatchUpdate(this,this.refresh);}
...
export class Counter{constructor(){this.times=[];this.values=[];}
...
const index=this._recordIndexAt(x);const value=Number.withThousandsSeparator(this.counter.values[index]);this._value.textContent=Common.UIString.UIString(this._currentValueLabel,value);const y=this.graphYValues[index]/window.devicePixelRatio;this._marker.style.left=x+'px';this._marker.style.top=y+'px';this._marker.classList.remove('hidden');}
...
export class Calculator{setZeroTime(time){this._zeroTime=time;}
