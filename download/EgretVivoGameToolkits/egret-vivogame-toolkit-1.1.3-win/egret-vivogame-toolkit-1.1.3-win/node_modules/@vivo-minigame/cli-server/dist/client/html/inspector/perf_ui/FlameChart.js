...
const groups=this._rawTimelineData.groups;this._keyboardFocusedGroup=groupIndex;this._scrollGroupIntoView(groupIndex);const groupName=groups[groupIndex].name;if(!groups[groupIndex].selectable){this._deselectAllGroups();UI.ARIAUtils.alert(ls`${groupName} hovered`,this._canvas);}else{this._selectedGroup=groupIndex;this._flameChartDelegate.updateSelectedGroup(this,groups[groupIndex]);this._resetCanvas();this._draw();UI.ARIAUtils.alert(ls`${groupName} selected`,this._canvas);}}
...
_onKeyDown(e){if(!UI.KeyboardShortcut.KeyboardShortcut.hasNoModifiers(e)||!this._timelineData()){return;}
...
let colorIndex=parsedColorCache.get(color);if(colorIndex===undefined){const rgba=Common.Color.Color.parse(color).canonicalRGBA();rgba[3]=Math.round(rgba[3]*255);colorIndex=colors.length/4;colors.push(...rgba);if(colorIndex===256){colorArray=new Uint16Array(colorArray);}
...
const groupOffsets=this._groupOffsets;const lastGroupOffset=Array.prototype.peekLast.call(groupOffsets);const colorUsage=UI.UIUtils.ThemeSupport.ColorUsage;context.save();context.scale(ratio,ratio);context.translate(0,-top);const defaultFont='11px '+Host.Platform.fontFamily();context.font=defaultFont;context.fillStyle=self.UI.themeSupport.patchColorText('#fff',colorUsage.Background);this._forEachGroupInViewport((offset,index,group)=>{const paddingHeight=group.style.padding;if(paddingHeight<5){return;}
...
export const HeaderHeight=15;export const MinimalTimeWindowMs=0.5;export class TimelineData{constructor(entryLevels,entryTotalTimes,entryStartTimes,groups){this.entryLevels=entryLevels;this.entryTotalTimes=entryTotalTimes;this.entryStartTimes=entryStartTimes;this.groups=groups;this.markers=[];this.flowStartTimes=[];this.flowStartLevels=[];this.flowEndTimes=[];this.flowEndLevels=[];this.selectedGroup=null;}}
export class FlameChartDataProvider{minimumBoundary(){}
totalTime(){}
...
export class FlameChartMarker{startTime(){}
color(){}
title(){}
draw(context,x,height,pixelsPerMillisecond){}}
export const Events={CanvasFocused:Symbol('CanvasFocused'),EntryInvoked:Symbol('EntryInvoked'),EntrySelected:Symbol('EntrySelected'),EntryHighlighted:Symbol('EntryHighlighted')};export const Colors={SelectedGroupBackground:'hsl(215, 85%, 98%)',SelectedGroupBorder:'hsl(216, 68%, 54%)',};export let Group;export let GroupStyle;