...
_onKeyDown(event){if(UI.UIUtils.isEditing()||!this._itemCount||event.shiftKey){return;}
...
const textLines=[];for(let i=startSelection.item;i<=endSelection.item;++i){const element=this._providerElement(i).element();const lineContent=element.childTextNodes().map(Components.Linkifier.Linkifier.untruncatedNodeText).join('');textLines.push(lineContent);}
...
const untruncatedContainerLength=Components.Linkifier.Linkifier.untruncatedNodeText(selectionNode).length;if(offset>0&&untruncatedContainerLength!==selectionNode.textContent.length){offset=untruncatedContainerLength;}
return chars+offset;}
...
export class ConsoleViewportProvider{fastHeight(index){return 0;}
itemCount(){return 0;}
