...
event.dataTransfer.setData('text/plain',treeElement.listItemElement.textContent.replace(/\u200b/g,''));event.dataTransfer.effectAllowed='copyMove';this._treeElementBeingDragged=treeElement;SDK.OverlayModel.OverlayModel.hideDOMNodeHighlight();return true;}
...
var node=treeElement.node();if(node.nodeName().toLowerCase()==="document"||node.nodeName().toLowerCase()==="body"||node.parentNode&&node.parentNode.nodeName().toLowerCase()==="body")
...
return true;return!!node.childNodeCount()&&!ElementsTreeElement.canShowInlineText(node);}
