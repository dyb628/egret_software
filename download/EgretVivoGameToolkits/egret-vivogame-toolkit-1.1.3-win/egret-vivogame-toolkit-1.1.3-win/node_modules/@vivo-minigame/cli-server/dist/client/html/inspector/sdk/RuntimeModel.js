...
_executionContextCreated(context){const data=context.auxData||{isDefault:true};const executionContext=new ExecutionContext(this,context.id,context.name,context.origin,data['isDefault'],data['frameId']);this._executionContextById.set(executionContext.id,executionContext);executionContext.hasCompileAndRunScript()
...
export class ExecutionContext{constructor(runtimeModel,id,name,origin,isDefault,frameId){this.id=id;this.name=name;this.origin=origin;this.isDefault=isDefault;this.runtimeModel=runtimeModel;this.debuggerModel=runtimeModel.debuggerModel();this.frameId=frameId;this._setLabel('');this._hasCompileAndRunScript=undefined;}
async hasCompileAndRunScript(){if(this._hasCompileAndRunScript===undefined){const result=await this._evaluateGlobal({expression:'global.compileAndRunScript',objectGroup:'',includeCommandLineAPI:true,silent:false,returnByValue:false,generatePreview:true},true,false);this._hasCompileAndRunScript=result.object.type==='function'}
