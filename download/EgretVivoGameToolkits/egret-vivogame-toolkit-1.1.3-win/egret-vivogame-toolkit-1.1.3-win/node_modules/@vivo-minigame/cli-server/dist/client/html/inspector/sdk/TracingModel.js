...
self.Common.console.error('Failed to find browser main thread in trace, some timeline features may be unavailable');return null;}
...
try{const payload=JSON.parse(result);callback(payload['args']['snapshot']);}catch(e){self.Common.console.error('Malformed event data in backing storage');callback(null);}}}
