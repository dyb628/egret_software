import*as Platform from'../platform/platform.js';import*as TextUtils from'../text_utils/text_utils.js';export class AcornTokenizer{constructor(content){this._content=content;this._comments=[];this._tokenizer=acorn.tokenizer(this._content,{onComment:this._comments});const contentLineEndings=Platform.StringUtilities.findLineEndingIndexes(this._content);this._textCursor=new TextUtils.TextCursor.TextCursor(contentLineEndings);this._tokenLineStart=0;this._tokenLineEnd=0;this._nextTokenInternal();}
